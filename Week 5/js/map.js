...
function mapCSV(date) {
	...
	csvdata.data.forEach(function (item, index) {
		if (item.Lat != undefined) {
			...
			let marker = L.circleMarker([item.Lat, item.Long], circleOptions).on(
				"mouseover",
				function () {
					this.bindPopup(
						`${item["Country/Region"]}<br>Total confirmed cases as of ${date}: ${item[date]}`
					).openPopup();
				}
			);
...
function createSidebarButtons() {
	...
	dates.forEach(function (item, index) {
		$(".sidebar-content").append(
			`<span onmouseover="mapCSV('${item}')" class="sidebar-item" title="${item}">●</span>`
		);
	});
}
